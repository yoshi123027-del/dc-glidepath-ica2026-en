% v15 merge: v8 title/abstract + v14_9 remaining manuscript text
...
\setmainfont[Path=/usr/share/texmf/fonts/opentype/public/lm/,BoldFont=lmroman10-bold.otf,ItalicFont=lmroman10-italic.otf]{lmroman10-regular.otf}
\setsansfont[Path=/usr/share/texmf/fonts/opentype/public/lm/,BoldFont=lmsans10-bold.otf]{lmsans10-regular.otf}
\setmonofont[Path=/usr/share/texmf/fonts/opentype/public/lm/]{lmmono10-regular.otf}
\setCJKmainfont[Path=/usr/share/texlive/texmf-dist/fonts/opentype/public/haranoaji/,BoldFont=HaranoAjiMincho-Bold.otf,ItalicFont=HaranoAjiMincho-Regular.otf]{HaranoAjiMincho-Regular.otf}
\setCJKsansfont[Path=/usr/share/texlive/texmf-dist/fonts/opentype/public/haranoaji/,BoldFont=HaranoAjiGothic-Bold.otf]{HaranoAjiGothic-Medium.otf}
\setCJKmonofont[Path=/usr/share/texlive/texmf-dist/fonts/opentype/public/haranoaji/]{HaranoAjiGothic-Regular.otf}
...
本稿は，現実的なDC投資制約の下で，四つの平均--分散方策を「初期の同じ期待終価」と「途中の同じ残高」という二つの基準で比較し，加入時と運用中の意思決定原理の違いが加入者の運用成果にどう現れるかを示す．

DCの運用設計では，同じ退職時の期待資産額を目指しても，低残高に至るリスク，高い成果を得る可能性，リスクを取る時期は異なり得る．また，途中の残高が観測された後は，その時点の残高を起点とする将来成果を評価する必要がある．本稿では，加入時と運用中という二つの意思決定場面を設定し，四つの動的平均--分散方策を比較する．
...
空売り禁止，破産制約，凸ポートフォリオ制約は，制約付き値関数，射影構造，自由境界を生む\citep{LiZhouLim2002,LiXu2016,BensoussanWongYamYung2014,GuanLiZhouZhou2022}．DCモデルで拠出と制約を同時に扱うと計算が複雑になるため，無制約解を実行可能範囲へ切り詰める実装も用いられる\citep{DiGiacintoFedericoGozziVigna2014,FerreiraMoriciVigna2025}．事後クリップは無制約問題の継続価値を保ったまま現在の制御だけを射影するため，制約を将来の値関数や均衡モーメントへ組み込む直接制約方策とは別の方策となる．本稿は四方策の比較を主分析とし，この実装差を補助分析として第\ref{sec:numerical-results}節，付録\ref{sec:strict-clip-results}，付録\ref{app:corrections}で検証する．

\paragraph{課題3（O3）：途中残高を固定した条件付き成果を評価すること．}
終端分布の無条件比較には，評価時点までに形成された状態間格差と，その後に残る運用リスクの双方が含まれる．DOMVの「ローリング」は残存期間問題を解き直す意思決定原理であるのに対し，本稿の条件付き評価は，任意の保存方策を同一時点・同一残高から継続したときの成果評価である．全分散の公式を使って両者を区別し，第\ref{sec:rolling-main}節で再較正後方策の条件付き分布を比較する．定義と計算法は付録\ref{sec:rolling-definition}に示す．

\paragraph{課題4（O4）：MVS拡張が解概念に与える影響を探索的に検討すること．}
MVSの時間整合的定式化には既存の理論がある\citep{MuTianYang2019}．同論文の定係数・無制約設定ではcTCMVSとcTCMVの均衡投資額が一致し，共通の無制約解を事後クリップした方策も一致することが知られている．付録\ref{sec:mvs}で，シミュレーション結果を発展的内容として記載し，追加検証を要する拡張として整理する．
...
以上の四点を組み合わせ，初期の共通平均と途中の共通状態という二つの比較軸を一つのDC分析の枠組みに統合することが本稿の統合的な貢献である．
\section{モデルと解概念}
\label{sec:model}
...
\begin{equation}
  ...
  \label{eq:Y-dynamics}
\end{equation}
となり，確定的拠出フローを形式上除去できる．この変換には二つの役割がある．第一に，$X_t+H_t$ を総年金富として解釈し，dTCMVの富依存リスク許容度を定義する．第二に，投資制約を外した補助問題を標準的なBlack--Scholes型の形に直し，クリップ近似の入力となる無制約解を容易に構成する．実際の投資可能額は $X_t$ であり，$Y_t$ は背景富を含む評価上の総年金富である．このため，直接制約フィードバックを定めるHJB，検証定理および基準数値計算は一貫して $X$空間で行い，$Y$空間は総年金富の解釈と無制約ベンチマークの構成に用いる．
...
\end{equation}
とする．これを時間の関数と見たものをグライドパスと呼び，状態依存のグライドパスについては，その平均を代表的なグライドパスとして用いる．この代表的なグライドパスを比較することが本論文における分析結果の一つの指標である．$X_t=0$ では $\pi_t=0$ となるため，$g_t$ は未定義とする．数値計算では $x=0$ を下端境界とし，負の離散遷移値を0へ切り上げる．拠出がある場合，次期以降は正の残高へ移り得る．正の低残高は，低残高側を細分化した非一様格子上で線形補間する．
...
\end{tabularx}
\end{table}
平均--分散最適化問題には時間非整合性があり，望ましい方策は意思決定時点によって変わり得る．したがって，まず何に対する最適性を求めるかを定める必要がある．
...
\]
には$(\E[W_T^{\pi}])^2$が含まれるため，通常の終端期待効用$\E[U(W_T^{\pi})]$とは異なる扱いが必要になる．PCMVとDOMVでは，固定ターゲットを持つ二次損失補助問題へ変換し，その補助問題に動的計画法を適用する．
...
cTCMVとdTCMVでは，将来の自己が採る方策を所与として，当期の短時間逸脱で目的を改善できない均衡を求める．前者の分散回避係数は一定であり，後者は現在の総年金富に依存する．ここでの時間整合性は，将来の自己との個人内均衡として定義される．
...
局所目的が適切な曲率を持つとき，直接制約方策も区間への射影で表せる．ただし，射影中心は制約付き値関数や均衡モーメントから決まるため，無制約方策を射影するクリップ近似とは構成が異なる．曲率条件が満たされない点では，端点を含む局所目的を直接比較する．
...
制約条件の扱いは，本稿の主要な比較軸の一つである．無制約解は，実行可能範囲へ射影することで計算負荷の小さいクリップ近似を構成できる．これに対し，直接制約フィードバックは，制約を将来の継続価値まで組み込んで方策を求める．本稿では，クリップ近似が直接制約フィードバックをどの程度再現するかを評価する．
...
共通の遷移核は，後退計算と前進計算の内部整合性を与える．一方，月次過程への近似精度は，状態格子，補間，離散化および格子間の確率質量配分にも依存する．そこで本文の主要値は，保存方策を別の月次Euler評価器へ移した独立評価で確認する．この検証は有限モデル上の評価器比較を対象とし，連続時間問題への収束は別の検証課題として扱う．検証結果とその範囲は付録\ref{app:corrections}に示す．
...
PCMVの埋込みターゲット，DOMVとcTCMVの分散回避係数，dTCMVの尺度係数，CPの定率は，それぞれの方策を求め直しながら期待終価84.78へ較正する．PCMV，DOMV，cTCMV，dTCMVで用いる係数は役割と尺度が異なるため，方策間の比較には係数値そのものではなく，較正後の成果分布と方策形状を用いる．
...
表\ref{tab:final-calibration-parameters}は，本文表\ref{tab:all-strategy-results}を生成した保存方策の最終値である．各パラメータは目的関数の中で異なる役割を持つため，選好の強さは係数値を横並びにするのではなく，較正後の方策と成果で比較する．保存先と再生成手順は付録\ref{app:reproducibility}に示す．
...
cTCMV・dTCMVの連続時間問題については，本稿固有の状態依存両側制約$0\leq\pi\leq X$の下での大域的均衡解の存在・一意性・正則性を今後の理論課題とする．本文で「最適方策」「均衡方策」と記す場合は，文脈上断らない限り有限状態・有限制御モデル上の方策を指す．連続時間問題に対する検証定理は，付録\ref{sec:theory}と付録\ref{app:technical-proofs}に示す正則性・可積分性条件の下で適用する．
...
本節の主要値は，再最適化・再較正した保存方策を独立100万経路で評価した結果である．方策自体は第\ref{sec:method-main}節のMGH後退計算で求め，独立Monte Carloではその方策を固定したまま成果分布を再評価する．
...
PCMVは標準偏差が最小，中央値が最大である一方，q05と下位5\%平均は5方策中で最も低い．終端富をターゲット近傍へ集中させる効果が強い反面，左裾指標では低い値を示す．DOMVとcTCMVは近い分布となり，cTCMVのq05はDOMVよりわずかに高く，下位5\%平均はDOMVの方が高い．dTCMVは標準偏差とq95が最大で，上方参加と下方リスクの双方を拡大する．CPはdTCMVに近い標準偏差とq95を持ち，下位5\%平均は5方策中で最大となった．したがって，方策の評価順位は標準偏差，分位点，裾平均のうち何を重視するかによって変わる．
...
図\ref{fig:all-glide}は，独立評価の各経路で実現したリスク資産比率を時点ごとに平均したグライドパスである．PCMV，DOMV，cTCMVは概して低下するが，低下速度は異なる．dTCMVの平均リスク資産比率は初期には上限制約に張り付き，その後急速に低下し，中盤で底を打った後，終盤に上昇する．CPは定義上一定である．いずれの曲線も，年齢に加えて，状態依存フィードバックと各方策が形成する残高分布を反映した平均的な運用経路である．
...
30年・残高45では，dTCMVはcTCMVより条件付き平均が5.76，q95が29.84高い一方，標準偏差は11.63大きく，q05は6.13，下位5\%平均は5.87低い．20年・残高20と35年・残高30では，q05および下位5\%平均の差は小さく，実務上は同程度の下方裾として扱うのが妥当である．三状態を通じて，現在残高，残存期間，評価指標によって，期待成果，上方参加，下方裾の組合せが変わることが確認できる．
...
MVSは，非凹性と数値安定性を含む追加検証を要する探索的拡張として付録\ref{sec:mvs}に整理する．dTCMVに見られるU字型のグライドパス形状は，高次モーメントを加えることで変化し得る．本稿の三層構造は理論・数値検証が整った平均--分散方策を対象とし，MVSの実務への接続は今後の研究課題とする．
...
\subsection{結論}
本稿は，加入者の目的，現在の状態，投資制約を与えたときに，DC年金のグライドパスを動的最適化から内生的に導く統一的な枠組みを構築した．平均--分散問題の時間非整合性を踏まえ，PCMV，DOMV，cTCMV，dTCMVを共通の市場・拠出・投資制約の下で比較すると，同じ期待終価に較正しても，終端分布，リスク取得時期，平均グライドパスは異なった．この結果から，DC年金の最適グライドパスは，採用する意思決定原理と加入者状態に応じて内生的に定まると解釈できる．

投資制約の実装では，無制約解を実行可能範囲へ射影するクリップ近似と，制約を継続価値へ組み込む直接制約方策を比較した．両者の近さは解概念によって異なる．再現精度が高い場合にはクリップ近似が実装を簡素化する一方，差が大きい場合には直接制約最適化が方策形状と成果分布の双方に重要となる．したがって，実務上の実装方法は，計算の簡便さと，加入者成果・リスク取得時期の再現精度を併せて選択する必要がある．

途中時点については，時点と現在残高を固定した条件付き成果を比較した．cTCMVとdTCMVでは，同じ現在残高から出発しても，期待成果，上下方分位点，裾リスクの組合せが異なり，その差は残存期間と残高状態によって変化した．ローリング条件付き成果指標は，時点0までに形成された状態間格差と，現在状態から先に残る運用リスクを分けて示すため，状態依存型の運用を加入者単位で説明する評価軸となる．

これらの結果を実務上の役割へ接続する設計仮説として，cTCMVを標準デフォルト，DOMVを定期的な運用見直し，dTCMVを総年金富に基づく個別化助言とする三層構造を提案した．PCMVは，目標終価と平均--分散のトレードオフを整理する設計ベンチマークとして三層を支える．MVSは，高次モーメントがリスク取得時期を変える研究拡張として，今後の理論・数値検証へつなげる．

本稿の中心的な結論は，DC年金の運用設計を「どの形のグライドパスを選ぶか」から，「どの意思決定原理の下で，現在の加入者状態に対してどの程度のリスクを取るか」へ捉え直す点にある．この問いを起点とすれば，グライドパスを内生的に導出し，制約実装と成果分布を検証したうえで，標準運用，定期見直し，個別化助言という異なる実務層へ接続できる．
...
\end{equation}
と整理する．クリップ近似は無制約解を実行可能範囲へ射影して構成する．直接制約フィードバックは，将来の制約拘束を継続価値へ反映して構成する．式\eqref{eq:comparison-ladder}の二つは，制約の取り込み方が異なる比較対象である．
...
\begin{equation}
  ...
  \label{eq:クリップ-def}
\end{equation}
を構成し，クリップ近似と呼ぶ．これはDC制約を満たす実装可能方策であり，無制約問題の継続価値を用いた事後射影として，対応する制約付きHJB方程式または制約付き拡張HJB系から得る直接制約方策と比較する．
\end{definition}
...
cTCMVおよびdTCMVでは，大域的な均衡解の構成が時間非整合制御に特有の不動点問題となる．\citet{BensoussanWongYamYung2014}は空売り禁止という片側制約の下で均衡を構成している．本稿の状態依存両側制約 $0\leq\pi_t\leq X_t$ については追加の理論検討が必要であるため，定理\ref{thm:ctcmv-verification}および定理\ref{thm:dtcmv-verification}は，十分に正則な大域的均衡候補を所与とした局所検証結果として位置付ける．この制約下における均衡解の存在・一意性および正則性の証明は，今後の課題とする．
\end{assumption}
...
\end{equation}
と置く．ここで $\rho_d$ は較正時に一度定め，時点および状態によらず固定する尺度パラメータであり，各時点・各状態の自己が実際に用いる分散回避係数は状態依存の $\Gamma(t,x)$ である．したがって，添字によって時点0の係数を表すのではなく，$\rho_d$ と $\Gamma(t,x)$ を明確に区別する．この仕様では，同じDC口座残高 $x$ であっても，将来拠出やDB給付の現在価値 $H_t$ が大きい加入者ほど分散回避係数が低くなる．この仕様により，dTCMVは加入者ごとの総年金富をリスク配分へ反映する個別化助言として解釈できる．

\begin{remark}[$\Gamma$と投資上限の役割]
$\Gamma(t,x)$ は分散回避係数の尺度を表し，投資可能額は $0\leq\pi_t\leq X_t$ によって定まる．背景富はリスク負担能力に反映される一方，実際のリスク資産投資額はDC口座残高の範囲に収まる．この二つを分けることで，総年金富に基づくリスク許容度とDC口座内の実行可能性を同時に表現する．
\end{remark}
...
CPは，$\pi^{cp}(t,x)=\theta_{cp}x$ と置く実行可能な定率ベンチマークであり，HJBから導く四つのMV方策とは役割が異なる．本稿では，四つのMV戦略と期待終端富をそろえるように $\theta_{cp}$ を較正し，同じ期待値の下で分布形状を比較する．この設定は，異なる戦略を共通平均の下で比較する \citet{vanStadenDangForsyth2021} の考え方に対応する．
...
\begin{proof}
PCMV/DOMVについては定理\ref{thm:embedded-pcmv}，cTCMVについては定理\ref{thm:ctcmv-projection}，dTCMVについては定理\ref{thm:dtcmv-projection}を適用すれば，各射影表示を得る．三者の共通点は，局所ハミルトニアンが有効曲率の符号条件の下で凸または凹の二次式となることである．一方，射影中心を構成する関数は解概念ごとに異なり，いずれも対応する制約付きHJB方程式または制約付き拡張HJB系の解から計算される．この結果から，共通化されるのは射影の形式であり，射影中心は各解概念の制約付き値関数または均衡モーメントによって定まる．
\end{proof}
...
付録\ref{sec:theory}で定式化した四つの制約付きMV問題を同一条件で後退計算・前進評価するため，Gauss--Hermite条件付き期待値，格子上の動的計画および確率質量伝播を共通化し，この計算アーキテクチャを\emph{統一Markov--Gauss--Hermiteエンジン}と呼ぶ．各構成要素には既存の数値理論があり，Gauss--Hermite求積は標準的な決定論的数値積分\citep{AbramowitzStegun1972,Judd1998}，制御拡散に対するMarkov近似と半Lagrange型後退計算には一般論\citep{KushnerDupuis2001,FalconeFerretti2014}，MV資産配分のHJB数値解には先行研究\citep{WangForsyth2012,WangForsyth2012Comparison}がある．本稿では，これらを四つの解概念，直接制約方策とクリップ近似，終端・ローリング条件付き分布，MV/MVS入れ子検証へ一貫して適用する．
...
この共通化は，次の四つの役割を持つ．第一に，\emph{比較条件の統制}である．解概念間または直接制約フィードバックとクリップ近似の結果差を比較するとき，条件付き期待値の計算法，状態・制御格子，前進分布の評価法を共通化するため，差を主として目的関数・時間整合性・方策構成へ帰属できる．第二に，\emph{理論から加入者成果への橋渡し}である．検証定理が特徴づける局所フィードバックを，終端富分布，下方裾およびローリング条件付き分布まで同じ核で伝播できる．第三に，\emph{自由境界と射影差の再現可能な診断}である．内側ループに乱数モンテカルロ法を置かないため，候補制御間の小さな差や制約境界の位置が標本ノイズで変動しない．第四に，\emph{数値ガバナンス}である．後退モーメントと前進分布の整合性，確率質量保存，$\eta=0$におけるMVへの入れ子回帰検証などを，共通実装の中で実行できる．

\noindent\textbf{数値的貢献の範囲．}
本稿の数値的貢献は，標準的な数値部品を一つの後退・前進アーキテクチャに統合し，四つの制約付きMV解概念，直接制約フィードバックとクリップ近似の比較，MV/MVS入れ子，後退方策計算，終端・ローリング分布の前進評価，実務ガバナンスへ共通利用する点にある．前進計算では，求積節点から構成した離散遷移核に沿って確率質量を決定論的に伝播する．独立Monte Carlo評価は，この内部評価とは別の評価器として用いる．
...
\begin{equation}
  ...
  \label{eq:time-grid}
\end{equation}
とする．本文の基準計算は，40年間の拠出と運用見直しを月次で行う離散時間モデルとして $N_t=480$（$\Delta t=1/12$）を採用する．これは制度上の意思決定頻度に対応するモデル仕様である．連続HJBに対する収束性は別の検証課題とする．多数のパラメータを横断する感応度分析では計算負荷を抑えるため $N_t=80$ のスクリーニング仕様を用い，本文の基準計算 $N_t=480$ とは目的を分ける．後退計算は $n=N_t-1,N_t-2,\ldots,0$ の順に，分布伝播は $n=0,1,\ldots,N_t-1$ の順に実行する．
...
低残高領域を細分化する理由は三つある．第一に，許容投資額区間 $[0,x]$ は $x\downarrow0$ とともに縮退し，投資額 $\pi$ の小さな補間誤差が比率 $g=\pi/x$ では大きく増幅される．第二に，$x=0$ では全ての比率候補が同じ投資額 $\pi=0$ を与えるため，比率は経済的に識別されず，実装上の同点処理によって便宜的に0が選ばれ得る．第三に，dTCMVでは $\Gamma(t,x)=\rho_d/(x+H_t)$ が状態に依存するため，低残高点の制御誤差が背景富と上限制約の相互作用を通じて後続時点へ伝播する．したがって，最初の正の格子点が初期残高より大幅に高い粗い格子では，$x=0$ と遠い正の格子点の間の補間により，加入初期のリスク資産比率を人工的に0へ押し下げることがある．このため，低残高側の格子を十分に細かくし，比率ではなく投資額 $\pi=xg$ を補間する．全時点の格子精度は，初期点の処理に加えて格子幅・上端境界・補間方法を含めて検証する．
...
前節の理論では，滑らかな値関数またはモーメント関数が与えられたとき，直接制約フィードバックが局所無制約候補を $[0,x]$ に射影する形で表されることを示した．数値実装では，候補制御を $[0,x_i]$ 上で直接探索し，境界拘束領域と内点領域を同じ手順で計算する．射影公式は，計算結果を診断し，自由境界と射影差を解釈するために使う．
\end{remark}
...
具体的には，1期間先の条件付き期待値を直接評価し，そこから得られる遷移核を用いて動学的な後退計算を構成する．正規ショックはGauss--Hermite求積で評価し，同一の遷移核を後退計算と前進確率質量伝播の双方に用いる．さらに，格子精緻化，境界質量診断，後退モーメントと前進分布の整合性によって数値解を検証する．これにより，近似誤差を時間・状態・制御格子，補間，求積次数という個別の要因に分けて監視できる．
...
\end{split}
\end{equation}
ここで，$\mu_j$と$s_j$は $W_T^{j;t,x}$ の条件付き平均と標準偏差，$q_{j,\alpha}$は条件付き分位点，$g_j$と$\pi_j=xg_j$は現在のリスク資産比率と投資額である．以下では式\eqref{eq:rolling-outcome-profile}を\emph{ローリング条件付き成果指標}と呼ぶ．この指標ベクトルにより，期待成果，上下方リスク，分布の非対称性，現在の投資行動を同じ条件付き分布からまとめて示す．
...
現在残高を $x$ に固定するローリング条件付き評価では，$\mu_j(t,x)$，$v_j(t,x)$ および条件付き裾指標を直接評価し，その状態から先の成果分布を取り出す．無条件評価は制度開始時点の設計評価，ローリング評価は観測済みの現在状態からの将来見通しに対応し，両者は全期待値・全分散の公式によって接続される．

共通平均の下でPCMVは，時点0における総終端分散を最小化するよう設計されるため，無条件比較では分散圧縮が最も明確に現れる．これはPCMVを設計ベンチマークとして用いる根拠となる．dTCMVは，現在残高と背景富に応じて期待値，上方参加，下方リスクの組合せを更新する時間整合的な状態依存フィードバックである．したがって，PCMVは時点0の設計効率，dTCMVは現在状態から先の条件付き成果という異なる意思決定場面に対応し，それぞれに適した評価尺度を用いる．
...
さらに，戦略間の開始残高差と個別化効果を切り分けるため，dTCMV自身の途中残高分布から得た同一の開始状態 $(t_n,q_{n,k}^{d})$ に対して，dTCMVとcTCMVの双方を満期まで継続する共通状態比較を行う．自己経路比較は各戦略を実際に採用した加入者の経験を表し，共通状態比較は「現在の残高と残存期間が同じ加入者に対して，総年金富依存の分散回避係数を追加すると何が変わるか」を識別する．両者を併用することで，dTCMVの個別化効果と途中残高分布の違いを分けて評価できる．

\subsubsection{ローリング評価が必要な理由}

時点0から見た終端分布は，制度加入時に選ぶ運用方針の全体像を評価するために不可欠である．特に，共通平均の下で総終端分散を最小化するPCMVがこの評価で強い結果を示すことは，その定義と整合する．代表的な既存比較でも，PCMVの分布圧縮が際立つ一方，富依存TCMVは下方領域で定率戦略に劣り得ることが示されている\citep{vanStadenDangForsyth2021}．時点0評価では，dTCMVの大きな分散と下方リスクが強く表れる．

しかし，式\eqref{eq:total-variance-rolling}が示すように，時点0の総分散は，途中時点から先の条件付き分散だけでなく，その時点までに形成された状態間格差も含む．DC加入者は，10年後，20年後，30年後と運用を継続する中で現在残高を観測し，その状態を起点に将来の運用を判断する．この評価日時点では，過去に形成された状態格差と，現在状態から先に残る条件付きリスクを分けて把握することが有用である．
...
本付録では，市場・給付・拠出前提の変更に対して，平均グライドパスの水準と形状がどの方向へ動くかを確認する．計算は$N_t=80$（40年間を80分割），非一様101点状態格子，9点制御候補，3点Gauss--Hermite求積によるスクリーニングである．分散回避係数などは基準値に固定し，前提変更ごとに共通期待終価へ再較正していない．本分析は，方策形状が前提変更に対してどの方向へ動くかを確認するスクリーニングとして位置付け，本文の終端分布順位と連続時間問題の理論検証は別に扱う．
...
\paragraph{確定的終端給付．}
図\ref{fig:sensitivity-D}では，$D_T=0$と，拠出を安全資産で積み立てた終価に等しい確定給付を比較する．PCMV，DOMV，cTCMVの曲線はほぼ重なる一方，総年金富に応じて分散回避度が変わるdTCMVでは方策形状が変化する．この結果は，確定給付の影響が解概念によって異なることを示す．確率的なDB給付や所得リスクへの拡張は今後の課題である．
...
平均グライドパスは，市場前提，将来拠出，背景富・給付，現在状態に応じて内生的に変化する．また，cTCMVでは実線と点線が概して近い一方，他の解概念では前提や時期によって差が広がる．クリップ近似が直接制約方策をどの程度再現するかも，解概念と各方策が生成する状態分布に依存する．

\clearpage

\section{平均--分散--歪度への拡張}\label{sec:mvs}
本付録では，MVSに関する探索的分析と補足図を示す．目的関数には標準化歪度ではなく三次中心モーメントを用いるため，正の係数の効果は尺度と離散化に依存する．以下では原実装の係数感応度を示し，再較正後の共通平均MV方策との比較とは分けて解釈する．
...
さらに，\citet{MuTianYang2019}の定係数Black--Scholes型無制約問題における閉形式結果を，本稿が第1層の基準とする定数分散回避型へ適用すると，cTCMVSの無制約均衡投資額は対応するcTCMVと一致する．したがって，この共通無制約解を同じ区間$[0,x]$へ事後的に射影して構成するクリップ近似も一致する．無制約解とクリップ近似では，正の歪度選好を追加してもcTCMVSの投資額はcTCMVと一致する．直接制約問題では，制約拘束確率と条件付き高次モーメントが現在残高に依存するため，この相殺構造をそのまま用いることはできない．正の歪度係数を持つ直接制約cTCMVSについては，拡張HJB系，一致条件，自由境界の特徴づけを今後の課題とする．
...
図の一部に見られる細かな変動は，非凹な局所目的の下で，制御格子上の大域探索，状態補間，前進分布の微小な変化によって選択制御が切り替わった可能性を含む．$\eta_0=0.5,1.0,2.0$は，方策形状の係数感応度を確認するための探索点として設定した．

PCMV--MVSおよびDOMV--MVSについても計算し，現段階では探索的結果として本文の運用提案から分けて扱う．小さい歪度係数ではMV版とほぼ重なる一方，係数が一定範囲を超えると，とくにPCMV--MVSでリスク取得時期が急に切り替わる数値的分枝が現れた．DOMV--MVSの変化は相対的に小さいものの，終盤の方策がターゲット探索，ラグランジュ乗数，制御格子および補間へ敏感であった．これらの結果は，\path{archive/legacy_v5/supplementary/figures/fig_pcmv_domv_mvs_recalibrated_N80_glidepaths.png}，終端CDFおよび係数応答図として公開リポジトリに残す．再現可能な探索結果を公開することには，将来より頑健な解法が得られた際の比較基準を提供する価値があるが，現時点で加入者向け運用へ適用するには不十分である．
...
まず，保存方策が各格子点で$0\leq\pi\leq x$を満たすかを確認する．次に，固定ターゲットのBellman最小化，DOMVの候補ターゲット比較，またはTCMVの一時点逸脱目的が，採用した候補集合内で最適化・均衡条件と整合するかを確認する．最後に，別の状態格子や独立な正規ショックで評価し，達成平均と分布指標の感応度を確認する．この検証では，実行可能性，有限モデル上の最適化・均衡条件，独立評価器との近似精度をそれぞれ別の診断として確認する．

共通の遷移核により有限Markovモデル内部の整合性を確認し，月次過程への近似精度は独立評価で検証する．格子間の値を隣接点へ確率配分すると追加分散が生じるため，本文の主要結果には再最適化・再較正後の方策を用い，粗い状態格子による結果は監査記録として本付録と公開リポジトリに残す．
...
再最適化では，市場係数と月次の意思決定頻度を保ち，状態・制御格子を細かくして方策を求め直したうえで，共通期待終価へ再較正する．原表との差には，標本誤差に加えて方策と較正条件の更新が反映される．

独立検証用に，状態上限600，状態点数6001（間隔0.1），制御比率129点，GH7点で直接制約方策を再計算した．DOMVのターゲットは0から650まで2刻みとした．月次480期，$r=0.015$，$\beta=0.04$，$\sigma=0.18$，年額拠出1，初期残高$1/12$，$D_T=0$は原計算と共通である．較正の評価には別の状態間隔0.025とGH31点を用い，期待終価84.78からの最大差は0.003547であった．最適性と均衡性の数値的な検証は，採用した有限モデル上で行う．

\subsection{MGH前進評価と独立Monte Carlo評価}\label{app:validation-evaluators}

\subsubsection{評価器比較の目的}

本節では，現行の5方策の保存済み投資額配列を共通の入力として，MGH前進質量伝播と独立100万経路Euler--Monte Carloによる終端分布および平均グライドパスを比較する．方策の再最適化，係数の再較正，市場・拠出前提の変更は行わない．MGHは方策計算と同じ状態上限600，6001状態点，状態間隔0.1，GH7点，月次480期を用いる．較正時の別評価格子（間隔0.025，GH31点）とは区別する．初期状態は式\eqref{eq:exact-initial-glide}に従い，正確な$x_0=1/12$から最初の一期間を遷移させる．\path{results/validation/recalibration/checks.json}では初期質量を二点へ分割していたため，本節の前進値とは初期化が異なる．

MCには本文表\ref{tab:all-strategy-results}と図\ref{fig:all-glide}の生成元である\path{results/current/independent_mc.npz}を用いる．乱数種20260912，100ブロック（各ブロックの種は20260912に1009の倍数を加算），100万経路，方策間の共通乱数，上端切詰めなしという既存仕様を維持する．保存投資額を線形補間し，上端を超えた場合は上端の投資比率で継続する．全方策の主要統計量を既存CSVと照合した．同じ乱数種で100万経路を再生成した検算では，保存終価との経路別最大差は$5.90\times10^{-9}$であり，本文には元の保存MC値を引き続き用いる．方策・係数・元MCデータのSHA--256は検証記録に保存した．
...
分布全体の差として，両CDFの全支持点における左右極限を比較し，$D_{\mathrm{KS}}=\sup_z|F_{\mathrm{MGH}}(z)-F_{\mathrm{MC}}(z)|$を求める．ここでは$D_{\mathrm{KS}}$を分布間の距離指標として用い，検定の$p$値は算出しない．最大値はPCMVの0.008514で，他の4方策は0.001257から0.002079である（表\ref{tab:evaluator-distances}）．PCMVでは上側に集中する分布を格子幅0.1の離散CDFとして表す際の段差にも注意する必要があり，図には上側の拡大表示を添えた．
...
平均グライドパス差はdTCMVが最大で，34.75年時点の0.5889 pp，MAEは0.0704 pp，RMSEは0.1224 ppである．CPでは定率$\theta_{\mathrm{CP}}=0.4688842166$と両評価器の平均比率が丸め誤差の範囲内で一致し，評価器間の最大差は$8.6\times10^{-12}$ pp未満である．保存された状態依存フィードバック方策は両評価器で共通である．平均グライドパスの差は，各評価器が生成する状態分布で同じ方策を加重した結果として生じる．

\subsubsection{検証結果の要約}

今回の比較では，終端分布と平均グライドパスの主要な形状が両評価器で概ね一致し，本文の独立MC結果と意思決定原理に基づく設計上の結論を支持した．平均グライドパス差はdTCMV，最大CDF差はPCMVで最大となり，評価器差は指標ごとに現れ方が異なる．CPは平均比率が一致しても終端分布には差があり，状態離散化・質量配分等の評価器差を切り分ける基準となる．

各月の質量総和誤差は$2.2\times10^{-14}$未満，負の質量と下端質量はゼロであった．最大上端質量はCPの$6.43\times10^{-8}$，dTCMVでは$3.16\times10^{-9}$である．前進の平均・二次モーメントは同じ保存方策の後退評価と整合し，一期間ごとの二次モーメントには質量配分による追加分散を含めて検算した．上端到達質量は小さく，観測された分布差には状態離散化や質量配分など他の近似要因も寄与している．

現行CPの標準偏差はMGHで31.7442，MCで31.6874であり，相対差は約0.18\%である．付録\ref{app:legacy-corrections}に保存した旧粗格子CPの約19.5\%の乖離から大きく縮小した．旧方策と現行方策では較正条件等が異なるため，この数値は同一方策の改善率ではなく，現行評価器間の整合性を示す診断値として扱う．本検証の対象は有限モデルと独立評価器の比較であり，連続時間問題への収束と一様誤差限界は今後の検証課題である．

\clearpage
\subsection{格子・境界・離散化に対する頑健性}\label{app:validation-robustness}
状態上限の感応度は残るが，dTCMVを上限900で再較正した標準偏差は約32.29であり，本文の32.31と近い．

第\ref{app:legacy-corrections}節の旧格子検証では，原計算で用いた状態格子と分布配分に精度上の問題が確認された．改善には，求積次数，状態・制御格子，上端境界を個別に点検し，誤差要因に応じて調整する必要がある．

原計算の局所精緻化と追加の制御監査は，指定した時点・残高における有限モデル上の検査である．全状態域の大域最適化と一様誤差限界は別途検証を要するため，次節で有限モデルの検証範囲と連続モデルに残る課題を整理する．

\subsection{数値結果および理論的主張の範囲}\label{app:validation-scope}
数値的内部整合性は，採用した離散遷移核上で後退計算と前進計算が一致することを示す．有限モデル上の最適性・均衡性と，連続時間モデルにおける古典解の存在・一意性は，それぞれ別の検証対象として扱う．
...
従来の外部検証で用いたdTCMVの公表平均・標準偏差からの終端分布再構成は，均衡係数や後退方策の独立再現ではない．このため現行の付録\ref{app:four-model-validation}ではこの逆算を検証入力に用いず，van Staden et al. (2021) の問題設定をMGH後退計算で解き，得られた方策を前進評価して既知の方策・終端分布と比較する．ただし，dTCMVには原論文掲載式と目的関数から導く均衡条件の符号不整合が残るため，完全再現ではなく部分的外部検証と位置付ける．MVSについては，係数ゼロでMVへ戻る回帰試験と，正の係数に対する精度・最適性の検証を分けて扱う．原計算の図表とソースは，これらの検証範囲を追跡できる比較資料として保存する．
...
以下では，旧粗格子結果と符号訂正前のクリップ比較を監査履歴として保存し，現行結果へ至る訂正内容を記録する．

\subsubsection{粗い旧状態格子による追加分散}
原計算の表は，その実装が生成する有限Markovモデルの結果として再現できる．独立評価との比較により格子誤差が確認されたため，本文の主要結果は精緻化後の方策へ更新し，旧数値は訂正過程を追跡する監査記録として保存する．
...
原診断で用いた$\rho_d=1.193359$の無制約係数を訂正すると，時点0，10，20，30，35，39年の値は，0.0890，0.1819，0.3269，0.5729，0.7642，0.9719から，0.0676，0.1312，0.2316，0.4242，0.6165，0.9135となる．直接制約ソルバーはこの無制約係数を入力しないため，この訂正だけで直接制約方策は変わらない．旧dTCMVクリップの図表は符号訂正前の監査資料として保存し，訂正後の近似性能は現行実装で評価する．
...
表\ref{tab:all-strategy-results}の保存方策と独立評価値は\path{results/current/}にある．各方策の\path{*.json}と\path{*.npz}が表\ref{tab:final-calibration-parameters}の較正値と方策配列，\path{results/current/independent_mc.csv}が表\ref{tab:all-strategy-results}，\path{results/current/independent_mc.npz}が図\ref{fig:all-cdf}と図\ref{fig:all-glide}の再生成に用いる入力データとなる．ソルバーと評価器は\path{recalibration/}，検証判定は\path{results/validation/recalibration/acceptance.json}と\path{results/validation/recalibration/checks.json}に整理した．
...
\subsection{旧計算の監査記録}
粗い格子による原計算は\path{archive/legacy_v5/}に保存し，対応コミットを\texttt{d02af6cf693ec701396c2ef8879c4b1b52e25b70}に固定した．同フォルダにはソルバー，較正，ローリング評価，感応度分析，図表生成，妥当性検証を機能別に収録する．直前原稿との対応は\path{archive/papers/v11/}と\path{archive/exploratory/manifest_v11.json}から確認できる．詳細な依存関係，実行順序，出力ファイルは\path{README.md}，\path{archive/legacy_v5/scripts/README.md}，\path{CODEBOOK_JA.md}に記載した．

\subsection{探索的分析と補足図}
MVSおよび方策形状の感応度分析は\path{archive/legacy_v5/}と\path{archive/legacy_v5/supplementary/figures/}に整理した．補足図には，終端富密度，上限制約の拘束確率と自由境界，射影差，条件付き評価，主要パラメータに対する方策形状の感応度，MVS拡張を収録する．感応度分析は付録\ref{sec:strict-clip-results}，MVS拡張は付録\ref{sec:mvs}，旧計算の訂正と監査は付録\ref{app:corrections}に対応する．

\subsection{外部ベンチマークによるMGH後退・前進エンジンの検証}
\label{app:four-model-validation}

本稿の数値検証を三層に区別する．第一層は，同じ有限Markovモデル内での後退・前進モーメントの整合性である．第二層は，付録E.3の同一保存方策に対するMGH前進と独立100万経路Euler--Monte Carloの評価器比較である．本節は第三層として，\citet{vanStadenDangForsyth2021}の外部問題をMGH後退計算で解き，得られた方策を前進評価して，既知の方策・終端分布と比較する．閉形式から公表表を再計算するだけの検証とは区別する．

\subsubsection{ベンチマーク設定と一次資料}

原論文の式(2.1)，仮定3.1，式(5.1)および表5.1（掲載頁570，574，597，599）を確認し，$w_0=100$，$T=10$，$r=0.00623$，$\mu=0.0816$，$\sigma=0.1863$とした．拠出および終端給付はなく，リスク資産額$u$は空売り・借入れを許す無制約制御で，支払不能後も取引を継続する．目標平均125，250は原論文の明示的な入力条件である．本稿のDC制約$0\leq\pi\leq X$，40年，拠出および共通期待終価84.78は持ち込まない．

PCMVの埋込み目標$\gamma/2$，DOMVおよびcTCMVの分散回避係数は原論文の式(4.2)--(4.4)から求め，丸め前の値を全精緻化条件で固定した．DOMVは固定した分散回避係数の下で各時点・状態から残存期間問題を解き直し，最初の制御を接続する．dTCMVは式(2.10)の$\rho/(2w)$を用い，目標平均条件から$\rho$を決める．公表された標準偏差・中央値等を入力に用いず，公表表は計算後の比較にのみ使用した．設定，式番号，入力・出力の区別は公開仕様書に記録した．

\subsubsection{MGH後退・前進による再現方法}

既存DCエンジンの線形補間付き条件付きモーメント計算を共通関数に切り出し，DC計算と外部計算の双方から呼び出した．正規化GH節点も共用する．切出し前後のDC回帰試験では，三つの目的関数について方策・一次・二次モーメントの差はゼロだった．外部問題では$Y_t=e^{r(T-t)}W_t$を用い，PCMVの中心化した富には符号付き幾何格子，DOMV・cTCMVには等間隔格子，dTCMVには正の幾何格子を設けた．格子の平行移動・拡大縮小に関する対称性を用いて後退モーメントの格納量を減らすが，各候補制御の評価は共通MGH関数による．この縮約と格子上のモーメント配列を直接補間する計算を54点で照合した．

PCMVは二乗損失を数値最小化し，DOMVはその残存期間・埋込み目標の族について数値的にスカラー化目的を最大化する．cTCMV・dTCMVは将来の後退モーメントを与えて現在の均衡制御を数値探索する．解析方策を後退計算へ代入してはいない．制御範囲全体の257点探索と最良候補近傍の区間最適化を併用し，探索精度も別途変更した．前進計算は線形補間の転置に当たる非負質量配分である．初期富が格子外点であることを含めて一次・二次モーメントを照合した．

dTCMVでは正の比例方策の枝について，各区間で比率を固定したGBM遷移を使う．したがって，外部計算はDC固有の制約・遷移をそのまま呼び出したものではなく，共通モーメント核を用いた無制約問題向け実装である．この検証だけでDC固有の全コード経路まで検証したとはしない．

\paragraph{dTCMVの原論文記載に関する留保．}
原論文の式(3.5)には指数内の積分として$r+(\mu-r)\theta-\sigma^2\theta^2$が記載されている．一方，式(2.10)の目的関数から，$E[W_T\mid w]=aw$，$E[W_T^2\mid w]=cw^2$として導く均衡条件は
\[
 \theta=\frac{\mu-r}{\rho\sigma^2}
 \left\{\frac{a}{c}+\rho\left(\frac{a^2}{c}-1\right)\right\},
 \qquad
 \frac{a}{c}=\exp\!\left[-\int_t^T
 \{r+(\mu-r)\theta(s)+\sigma^2\theta(s)^2\}\,ds\right]
\]
となり，二乗項の符号が異なる．本検証では両方を別々に計算して記録した．後者の積分を逆時間の初期値ゼロのODEとして解き，目標平均条件から$\rho$を求めると，目標125では8.228651，250では0.112272となる．このODE方策をMGHに入力せず，MGH後退は目的関数から独立に制御を求めた．以下のdTCMVの理論参照はこの均衡導出に基づく．掲載式の文字どおりの計算は，目標250で標準偏差204.478，均衡導出では446.751となり，公表値444とも残差がある．公表平均・標準偏差から対数正規分布を逆算する従来の方法は今回の検証には用いない．この資料上の不整合は未解決であり，dTCMVを原論文の完全再現とは位置づけない．

\subsubsection{四方策の終端分布・方策の比較}

表\ref{tab:vanstaden-all-mv-validation}に，事前に定めた最細条件$N_t=1280$，GH7点，幾何格子の対数間隔0.00125，等間隔格子幅0.0625の結果を示す．各離散分布の分位点は累積確率の左逆関数，下位5\%平均は境界原子の必要質量だけを按分して求めた．図\ref{fig:external-mgh-cdf}は平滑化前のMGH離散CDFと連続時間参照CDFを比較する．最大差$D_{\rm CDF}=\sup_z|F_{\rm MGH}(z)-F_{\rm ref}(z)|$は全格子点の左右の極限で評価した．95\%分位点とCDFは理論比較であり，原論文表5.1の公表値とは称さない．

\begin{table}[htbp]
\centering
\caption{外部ベンチマークの公表値／MGH後退・前進値（最細条件）}
\label{tab:vanstaden-all-mv-validation}
\scriptsize
\setlength{\tabcolsep}{3.2pt}
\begin{tabular}{@{}rlrrrrr@{}}
\toprule
目標 & 方策 & 平均 & 標準偏差 & 中央値 & $q_{05}$ & 下位5\%平均 \\
\midrule
125 & PCMV & 125/124.999 & 9/9.136 & 127/127.512 & 113/113.227 & 97/97.374 \\
 & DOMV & 125/124.975 & 16/15.997 & 125/125.000 & 99/98.688 & 92/91.978 \\
 & cTCMV & 125/124.837 & 15/14.421 & 125/124.813 & 101/101.125 & 95/95.092 \\
 & dTCMV & 125/125.073 & 16/16.051 & 124/124.120 & 101/100.484 & 96/95.410 \\
\addlinespace
250 & PCMV & 250/249.991 & 71/70.628 & 269/269.419 & 159/159.013 & 37/36.434 \\
 & DOMV & 250/249.810 & 124/123.471 & 250/249.813 & 47/46.750 & $-5/-4.875$ \\
 & cTCMV & 250/250.015 & 112/112.239 & 250/250.000 & 65/65.375 & 19/18.499 \\
 & dTCMV & 250/250.877 & 444/447.275 & 123/122.732 & 17/17.159 & 11/11.294 \\
\bottomrule
\end{tabular}
\end{table}

公表された全出力指標に対する最大相対誤差は，両目標を通じてPCMV 8.145\%，DOMV 2.508\%，cTCMV 3.863\%，dTCMV 10.844\%だった．最大誤差の指標はそれぞれ無リスク終価の下回り確率，下位5\%平均，標準偏差，超過尖度である．公表値の整数・小数丸めの影響も含む比較であり，ゼロの公表値には相対誤差を定義せず絶対誤差を示した．例えばdTCMVの目標125の超過尖度は公表0.3に対して0.26747である．入力係数の丸め差はこれらの出力誤差から分離した．全公表指標と絶対誤差は公開CSVに残した．

方策自体は初期・中間・最終意思決定時点，富50・100・250の計9点で比較した．両目標を通じた最大相対誤差はPCMV 0.080\%，DOMV 0.231\%，cTCMV 0.879\%，dTCMV 9.522\%だった．dTCMVの最大誤差は目標125で残る．したがって，終端CDFが近いことだけをもって，各状態の方策も同じ精度で再現されたとはいえない．

\begin{figure}[p]
\centering
\includegraphics[width=0.96\textwidth]{figs/external_cdf_comparison.png}
\caption{外部問題の終端CDF比較．実線はMGH後退で求めた方策の前進分布，破線は連続時間参照分布．dTCMVの参照は目的関数から導いた均衡ODEであり，原論文の掲載式・公表表の完全再現ではない．表示範囲外も含む全離散質量からCDF最大差を計算した．}
\label{fig:external-mgh-cdf}
\end{figure}

\subsubsection{格子収束と境界・制御域の診断}

基準条件は$N_t=320$，GH7点，対数間隔0.005または等間隔幅0.25とした．目標250について時間刻み，状態格子幅，GH次数（3，7，15），状態域，制御域，最適化精度を一因子ずつ変更した48条件を，両目標の基準8条件と併せて保存した．さらに補間誤差の累積を考慮し，時間分割を2倍・4倍にすると同時に格子幅を1/2・1/4とする別の整合的精緻化経路16条件を追加した．後者は一因子検証の代わりではなく，組合せ誤差の確認である．全72条件を残し，最も公表値に近い格子を選択してはいない．

\begin{table}[htbp]
\centering
\caption{同時精緻化経路のCDF最大差と最細条件の方策誤差}
\label{tab:external-mgh-convergence}
\small
\begin{tabular}{@{}rlrrrr@{}}
\toprule
目標 & 方策 & $N_t=320$ & 640 & 1280 & 方策相対誤差（最大） \\
\midrule
125 & PCMV & 0.003702 & 0.003675 & 0.001092 & 0.080\% \\
 & DOMV & 0.006358 & 0.003559 & 0.001395 & 0.231\% \\
 & cTCMV & 0.007539 & 0.009329 & 0.005936 & 0.879\% \\
 & dTCMV & 0.023085 & 0.013041 & 0.004608 & 9.522\% \\
250 & PCMV & 0.003702 & 0.003675 & 0.001092 & 0.080\% \\
 & DOMV & 0.004542 & 0.002726 & 0.000876 & 0.231\% \\
 & cTCMV & 0.003233 & 0.001955 & 0.000203 & 0.010\% \\
 & dTCMV & 0.007803 & 0.003837 & 0.002027 & 0.947\% \\
\bottomrule
\end{tabular}
\end{table}

表\ref{tab:external-mgh-convergence}では概ねCDF差が縮小するが，cTCMVの目標125は単調でなく，dTCMVの同目標の方策誤差も中間条件7.95\%から最細条件9.52\%へ増えた．時間だけを細かくしたPCMVの目標250ではCDF差が0.01854へ増加した．これらは線形補間の格子位相と時間刻みの組合せへの感応性を示し，全面的な単調収束を主張できない．平均・SD・$q_{05}$・$q_{95}$を含む全条件の値は収束CSVに収録した．

状態域はPCMVの中心化した富の絶対値$10^7$，dTCMVの富$[10^{-8},10^7]$，DOMV・cTCMVの割引富$[-5000,5000]$とし，外側境界を半分・2倍にした．全条件で負の質量はなく，質量総和の最大誤差は$5.49\times10^{-14}$，累積境界逸脱確率の最大は$2.65\times10^{-19}$だった．状態域変更によるSD差は最大$2.54\times10^{-9}$，制御域半減・倍増による平均・SD差は最大$1.98\times10^{-6}$で，制御上限・下限に張り付く解はなかった．固定方策の後退・前進差は平均$1.77\times10^{-11}$，SD $2.58\times10^{-9}$以下だった．この範囲では境界切詰めより，時間離散化・補間・制御探索と原論文の定式化差が主な残差候補である．境界確率が小さいことだけで任意の高次モーメント誤差が小さいと保証するものではない．

\subsubsection{外部検証から確認できる範囲}

PCMV・DOMVでは，解析方策を入力せずに求めた後退方策と終端CDFの誤差が本精緻化経路で縮小した．cTCMVも目標250では良好に一致するが，目標125には非単調性と残差がある．dTCMVは公表出力を入力せずに均衡問題を独立計算したものの，掲載式との不整合および方策誤差が残るため，原論文に対する部分的外部検証とする．公表値への完全一致を一律の合格基準で宣言しない．

これらは，共通MGHモーメント核と外部問題向け後退・前進実装に対する数値的な裏付けであり，連続時間問題の最適性，一般的収束，DC制約下の全状態での精度，探索的MVSの妥当性を証明するものではない．本節の追加により本文のDC主要数値や三層運用の位置づけは変更していない．
...
PCMV，cTCMV，dTCMVの各局所ハミルトニアンは，有効曲率の符号条件の下で一変数の凸または凹二次式になる．そのため，無制約の停留点が $0$ 以下，$[0,x]$ 内部，$x$ 以上のいずれに位置するかにより，下限制約領域，内点領域，上限制約領域へ分かれる．停留点は制約付き値関数または均衡モーメントの微分から計算されるため，切替集合は未知関数に依存する自由境界である．この共通構造により射影形式は統一される一方，射影中心は制約付き値関数または均衡モーメントから定まり，クリップ近似とは異なる．
